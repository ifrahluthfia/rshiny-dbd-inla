...
#   R -e "plumber::plumb('api.R')$run(host='0.0.0.0', port=8000)"
#
# Atau buka file ini di RStudio lalu klik tombol "Run API"
# (RStudio otomatis mendeteksi file plumber jika ada komentar #* di atas fungsi)
#
# ============================================================
...
# Logika SAMA PERSIS dengan data_ready() di app.R asli
# ----------------------------------------------------------------
...
# Logika SAMA PERSIS dengan fit_one() di app.R asli
# ----------------------------------------------------------------
...
# Semua data.frame -> di-convert otomatis oleh jsonlite (dataframe="rows")
# ----------------------------------------------------------------
build_response <- function(res, d, covariates, spat_model) {

  # --- summary fixed + RR + posterior probability ---
...
function(req, res) {
  ...
  tryCatch({
    ...
    # ---- Fit model (dengan timeout manual sederhana via system.time) ----
    t0 <- Sys.time()
